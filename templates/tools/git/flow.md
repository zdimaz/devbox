...
```
<type>(<scope>): краткое описание задачи (Title — что нужно  сделать)               ← до 72 символов
...
Для автоматизации версионирования и генерации changelog рекомендуется использовать **Conventional Commits**. Формат сообщения:
...
- **style** – правки форматирования, пробелы, отступы (без изменения кода)
- **refactor** – рефакторинг без исправления багов и новых фич
- **perf** – улучшения производительности
- **test** – добавление/изменение тестов
- **chore** – служебные задачи (обновление deps, CI и т.д.)

Опциональный `<scope>` помогает группировать изменения (например `auth`, `ui`). Описание короткое, в повелительном наклонении, до 72 символов.

**Тело** (необязательно) – более детальное объяснение причины и подхода.

**Footer** используется для указания связанных задач, например `BREAKING CHANGE:` или `Closes #123`.
...
Add server‑side verification and store token in HttpOnly cookie.

Closes #42
```

```
fix(ui): correct heading color on dark theme

The previous color caused low contrast in dark mode.
```
...
Следуя этому шаблону, инструменты вроде `standard-version` смогут автоматически повышать версии и генерировать changelog.
